...
void nvbit_at_cuda_event(CUcontext ctx, int is_exit, nvbit_api_cuda_t cbid,
  ...
  if (cbid == API_CUDA_cuMemcpyHtoD_v2) {
    ...
    if (!is_exit) {
      ...
      sprintf(sass_buffer,std::string(traces_location+"/kernel-%d.sass").c_str(), kernelid);
      char mem_buffer[1024];
      sprintf(mem_buffer,std::string(traces_location+"/kernel-%d.mem").c_str(), kernelid);
      ...
      if (!stop_report) {
        ...
        grid_dimx = p->gridDimX;
	      grid_dimy = p->gridDimY;
	      grid_dimz = p->gridDimZ;
