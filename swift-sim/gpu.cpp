...
//    printf("%s\t%d\t%lld\t%lld\t%f\n", benchmark.c_str(), kernel_id, hardware, total_sm_cycles, delta(hardware, total_sm_cycles));
/*
 *  std::ofstream fout;
    fout.open("result.txt", std::ios::out | std::ios::app);
    fout << benchmark.c_str() << "\t" << kernel_id << "\t" << hardware << "\t"
	    << total_sm_cycles << "\t" << std::setiosflags(std::ios::fixed) << std::setprecision(2) << delta(hardware, total_sm_cycles) << "\n";
    fout.close();
*/
    ...
    for (auto &block: block_vec) {
        ...
        if (block.second->is_active(cycles)) {
            for (auto it = block.second->warp_vec.begin(); it != block.second->warp_vec.end(); ) {
                ...
                auto &warp = it->second;
                if (warp->warp_point >= warp->m_static_warp->m_insts.size() && warp->m_pending_mem_request_num == 0 && warp->pending_inst ==0) {
                    warp->active = false;
